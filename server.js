const express = require("express");
const cors = require("cors");
const app = express();

app.use(cors());
app.use(express.json());

let dataFromApp = {};
let rawDataFromESP = {};

app.get("/api", (req, resp) => {
  resp.json({ app: "Serverul Merge!" });
});

app.get("/api/rawDataFromESP", (req, resp) => {
  resp.json({ rawDataFromESP });
});

app.post("/api/deLaEsp", (req, res) => {
  const { id, temp, hum, setpoint, calibTemp, relay } = req.body;
  rawDataFromESP[id] = {
    id,
    temp,
    hum,
    setpoint,
    calibTemp,
    relay,
  };
  console.log(
    "Primesc de la ESP :",
    `Idul: ${id}`,
    `Temp: ${temp}`,
    `Hum: ${hum}`,
    `SetP: ${setpoint}`,
    `Calib: ${calibTemp}`,
    `Relay: ${relay}`,
  );

  //const espCmd = rawDataFromESP[id] || {};

  // const sp = Number(espCmd.setpoint ?? setpoint ?? 20);
  // const calibT = Number(espCmd.calibTemp ?? calibTemp ?? 0);
  // const hyst = Number(espCmd.hyst ?? 0.3);

  // const adjustedTemp = Number(temp) + calibT;

  // let relayState = dataStore[id]?.relay || 0;
  // if (adjustedTemp < sp - hyst) {
  //   relayState = 1;
  // } else if (adjustedTemp > sp + hyst) {
  //   relayState = 0;
  // }

  // dataStore[id] = {
  //   id,
  //   temp: adjustedTemp,
  //   hum,
  //   setpoint: sp,
  //   calibTemp: calibT,
  //   relay: relayState,
  // };

  // console.log(
  //   `ESP ${id} | tempBruta: ${temp} | calibT: ${calibT} | adjustedTemp: ${adjustedTemp.toFixed(1)} | sp: ${sp} | relay: ${relayState}`,
  // );

  // res.json({
  //   ok: true,
  //   cmd: {
  //     relay: relayState,
  //     setpoint: sp,
  //     calibTemp: calibT,
  //     hyst: hyst,
  //   },
  // });
});

// app.post("/api/server", (req, res) => {
//   const { id, cmd: incomingCmd } = req.body || {};

//   if (incomingCmd && id) {
//     cmdStore[id] = {
//       ...(cmdStore[id] || {}),
//       ...(incomingCmd.relay !== undefined && { relay: incomingCmd.relay }),
//       ...(incomingCmd.setpoint !== undefined && {
//         setpoint: incomingCmd.setpoint,
//       }),
//       ...(incomingCmd.calibTemp !== undefined && {
//         calibTemp: incomingCmd.calibTemp,
//       }),
//       ...(incomingCmd.hyst !== undefined && { hyst: incomingCmd.hyst }),
//     };

//     console.log(`Command for ${id}:`, cmdStore[id]);
//   }

//   const rooms = Object.values(dataStore);

//   console.log("Rooms trimise la app:", JSON.stringify(rooms));
//   console.log("dataStore:", JSON.stringify(dataStore));
//   console.log("Rooms:", rooms.length);

//   res.json({ rooms, cmd: id ? cmdStore[id] : {} });
// });

app.listen(9999, "0.0.0.0", () => {
  console.log("Server running on port 9999");
});
